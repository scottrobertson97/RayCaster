...
import type { GameState, Point, WallRayEntry } from '../types.js'

function isDrawableImage(image: HTMLImageElement | undefined): image is HTMLImageElement {
  return Boolean(image && image.complete && image.naturalWidth > 0 && image.naturalHeight > 0)
}

function getWallSamplePercent(ray: Point, isVertical: boolean, isUp: boolean, isLeft: boolean) {
  let percentage = 1
...
function drawBehindDoorRay(state: GameState, rayData: WallRayEntry) {
...
export function drawRayWall(state: GameState, rayData: WallRayEntry) {

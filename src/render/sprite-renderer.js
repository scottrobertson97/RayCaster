...
function getSpriteImage(assetRef) {
...
function advanceSpriteFrame(sprite, dt) {
...
export function drawSpriteEntity(entity, state, { fov = 90, pitchOffset = 0 } = {}) {
  const { sprite } = entity
  if (!sprite) return
...
export function drawSpriteEntity2D(entity, state) {
  if (!state.render.drawMap || entity.collected) return
...
function drawSpriteTracer(entity, state) {
  const ctx = state.render.mapCtx
  const player = state.world.player

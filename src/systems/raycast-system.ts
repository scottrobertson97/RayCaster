...
import type { GameState, Point, RayHit, WallRayEntry } from '../types.js'

function isIgnoredTile(ignoreTile: Point | null, tileX: number, tileY: number) {
...
export function castHorizontalRay(
  state: GameState,
  angle: number,
  ignoreTile: Point | null = null,
): RayHit & { isUp: boolean } {
...
export function castVerticalRay(
  state: GameState,
  angle: number,
  ignoreTile: Point | null = null,
): RayHit & { isLeft: boolean } {
  const map = state.world.map
  const player = state.world.player
...
export function applyFog(state: GameState, rayData: WallRayEntry) {
...
export function getFarRayPoint(state: GameState, angle: number): Point {
